...
#define RETURN_ERROR_DATA(DATA, i_rc) if ((DATA != NULL) && (DATA[EDB_RETURN_CODE] == 0)) { DATA[EDB_RETURN_CODE] = i_rc; } return i_rc;
#define SET_ERROR(i_rc) if ((iv_RealData != NULL) && (iv_RealData[EDB_RETURN_CODE] == 0)) { iv_RealData[EDB_RETURN_CODE] = i_rc; }
#define SET_ERROR_DATA(DATA, i_rc) if ((DATA != NULL) && (DATA[EDB_RETURN_CODE] == 0)) { DATA[EDB_RETURN_CODE] = i_rc; }
...
uint32_t ecmdDataBuffer::setCapacity(uint32_t i_newCapacity) {
  ...
  if (iv_Capacity < i_newCapacity) {
    ...
    if (iv_XstateEnabled) {
      ...
      if (iv_DataStr == NULL) {
        ETRAC("Unable to allocate Xstate memory for new databuffer");
        RETURN_ERROR(ECMD_DBUF_INIT_FAIL);
      }
    }
#endif
...
uint32_t ecmdDataBuffer::growBitLengthInternal(uint32_t i_newNumBits) {
  uint32_t rc = ECMD_DBUF_SUCCESS;
  ...
  if (getWordLength() > iv_Capacity) {
    ...
    if (iv_XstateEnabled) {
      ...
      if (temp == NULL) {
        ETRAC("Unable to allocate temp X-State buffer");
        RETURN_ERROR(ECMD_DBUF_INIT_FAIL);
      }
...
uint32_t ecmdDataBuffer::flipBit(uint32_t i_bit) {
  uint32_t rc = ECMD_DBUF_SUCCESS;
  if (i_bit >= iv_NumBits) {
    ETRAC("i_bit %d >= iv_NumBits (%d)", i_bit, iv_NumBits);
    RETURN_ERROR(ECMD_DBUF_BUFFER_OVERFLOW);
  }
#ifndef REMOVE_SIM
  if ((iv_XstateEnabled) && this->hasXstate(i_bit, 1)) {
    ETRAC("cannot flip non-binary data at bit %d", i_bit);
...
bool   ecmdDataBuffer::isBitSet(uint32_t i_bit) const {
  ...
  if (iv_XstateEnabled) {
    if (iv_DataStr[i_bit] != '1' && iv_DataStr[i_bit] != '0') {
      ETRAC("non-binary character detected in data at bit %d", i_bit);
...
bool   ecmdDataBuffer::isBitClear(uint32_t i_bit) const {
  ...
  if (iv_XstateEnabled) {
    if (iv_DataStr[i_bit] != '1' && iv_DataStr[i_bit] != '0') {
      ETRAC("non-binary character detected in data string");
      SET_ERROR(ECMD_DBUF_XSTATE_ERROR);
      return false;
    }
  }
...
uint32_t ecmdDataBuffer::insertBase(const ecmdDataBufferBase &i_bufferIn, uint32_t i_targetStart, uint32_t i_len, uint32_t i_sourceStart) {
  ...
  if (i_bufferIn.iv_XstateEnabled) {
    ...
    if (i_targetStart+i_len <= iv_NumBits) {
      strncpy(&(iv_DataStr[i_targetStart]), (genXstateStr(i_bufferIn, i_sourceStart, i_len)).c_str(), i_len);
    }
  } else if (iv_XstateEnabled) {
    // We have xstates, but the incoming buffer didn't, just grab the binary data 
    if (i_targetStart+i_len <= iv_NumBits) {
      strncpy(&(iv_DataStr[i_targetStart]), (genBinStr(i_bufferIn, i_sourceStart, i_len)).c_str(), i_len);
    }
  }      
#endif  
...
uint32_t ecmdDataBuffer::extractBase(ecmdDataBufferBase& o_bufferOut, uint32_t i_start, uint32_t i_len) const {
  ...
  if (iv_XstateEnabled) {
    enableXstateBuffer(o_bufferOut);
...
uint32_t ecmdDataBuffer::extract(uint32_t *o_data, uint32_t i_start, uint32_t i_len) const {
  ...
  if (iv_XstateEnabled) {
    ...
    if (hasXstate(i_start, i_len)) {
      ETRAC("Cannot extract when non-binary (X-State) character      present\n");
      RETURN_ERROR(ECMD_DBUF_XSTATE_ERROR);
    }
  }
#endif
...
std::string ecmdDataBuffer::genHexLeftStr(uint32_t i_start, uint32_t i_bitLen) const {
  ...
  if (i_start+i_bitLen > iv_NumBits) {
    ETRAC("i_start %d + i_len %d >= NumBits (%d)", i_start, i_bitLen, iv_NumBits);
    SET_ERROR(ECMD_DBUF_BUFFER_OVERFLOW);
    return ret;
...
std::string ecmdDataBuffer::genBinStr(uint32_t i_start, uint32_t i_bitLen) const {
    return genBinStr(*this, i_start, i_bitLen);
}

std::string ecmdDataBuffer::genBinStr(const ecmdDataBufferBase & i_bufferIn, uint32_t i_start, uint32_t i_bitLen) {
  int tempNumWords = (i_bitLen + 31) / 32;
  std::string ret;
  ...
  if (i_start+i_bitLen > i_bufferIn.iv_NumBits) {
    ETRAC("i_start %d + i_len %d >= NumBits (%d)", i_start, i_bitLen, i_bufferIn.iv_NumBits);
    SET_ERROR_DATA(i_bufferIn.iv_RealData, ECMD_DBUF_BUFFER_OVERFLOW);
  ...
  if ((i_start == 0) && (i_bitLen == i_bufferIn.iv_NumBits))
    tempData = i_bufferIn.iv_Data;
  else {
    tempData = new uint32_t[tempNumWords];
    /* extract iv_Data */
    rc = i_bufferIn.extract(&tempData[0], i_start, i_bitLen);
    if (rc) {
      if (data) // make sure we have the buffer before running delete on it.
      {
          delete[] data;
...
#ifndef REMOVE_SIM
  /* If we are using this interface and find Xstate data we have a problem */
  if ((i_bufferIn.iv_XstateEnabled) && hasXstate(i_bufferIn, i_start, i_bitLen)) {
    ETRAC("Cannot extract when non-binary (X-State) character present");
    SET_ERROR_DATA(i_bufferIn.iv_RealData, ECMD_DBUF_XSTATE_ERROR);
  }
#endif
...
std::string ecmdDataBuffer::genAsciiStr(uint32_t i_start, uint32_t i_bitLen) const {
  ...
  if ((i_start + i_bitLen) > iv_NumBits) {
    ETRAC("start %d + bitLen %d >= NumBits (%d)", i_start, i_bitLen, iv_NumBits);
    SET_ERROR(ECMD_DBUF_BUFFER_OVERFLOW);
    return ret;
  } else if (i_start >= iv_NumBits) {
    ETRAC("bit %d >= NumBits (%d)", i_start, iv_NumBits);
    SET_ERROR(ECMD_DBUF_BUFFER_OVERFLOW);
    return ret;
  } else if (i_bitLen > iv_NumBits) {
    ETRAC("bitLen %d > NumBits (%d)", i_bitLen, iv_NumBits);
    SET_ERROR(ECMD_DBUF_BUFFER_OVERFLOW);
    return ret;
  } else if (i_bitLen == 0) {
    ret = "";
...
std::string ecmdDataBuffer::genXstateStr(uint32_t i_start, uint32_t i_bitLen) const {
    return genXstateStr(*this, i_start, i_bitLen);
}

std::string ecmdDataBuffer::genXstateStr(const ecmdDataBufferBase & i_bufferIn, uint32_t i_start, uint32_t i_bitLen) {
  std::string ret;
  if (!i_bufferIn.iv_XstateEnabled) {
    ETRAC("Xstate operation called on buffer without xstate's enabled");
    SET_ERROR_DATA(i_bufferIn.iv_RealData, ECMD_DBUF_XSTATE_NOT_ENABLED);
    return ret;
  }
  ...
  strncpy(copyStr, &i_bufferIn.iv_DataStr[i_start], i_bitLen);
  copyStr[i_bitLen] = '\0';
...
uint32_t ecmdDataBuffer::enableXstateBuffer() {
    return enableXstateBuffer(*this);
}

uint32_t ecmdDataBuffer::enableXstateBuffer(ecmdDataBufferBase & io_buffer) {
  uint32_t rc = ECMD_DBUF_SUCCESS;
  if(!io_buffer.iv_UserOwned)
  {
    ETRAC("Attempt to modify non user owned buffer size.");
    RETURN_ERROR_DATA(io_buffer.iv_RealData, ECMD_DBUF_NOT_OWNER);
  }

  /* If it is already enabled, we don't do it again */
  if (io_buffer.iv_XstateEnabled) return rc;

  if (io_buffer.iv_NumBits > 0) {
    /* Check for null here to satisfy lint, but should always be NULL coming into this */
    if (io_buffer.iv_DataStr == NULL) io_buffer.iv_DataStr = new char[(io_buffer.iv_Capacity*32)+42];
    if (io_buffer.iv_DataStr == NULL) {
      ETRAC("Unable to allocate Xstate memory for new databuffer");
      RETURN_ERROR_DATA(io_buffer.iv_RealData, ECMD_DBUF_INIT_FAIL);
    }
    io_buffer.iv_DataStr[io_buffer.iv_NumBits] = '\0';

    /* Copy the raw data into the xstate buffer */
    uint32_t mask = 0x80000000;
    uint32_t curWord = 0;
    uint32_t numBits = io_buffer.getBitLength();

    for (uint32_t w = 0; w < numBits; w++) {
      if (io_buffer.iv_Data[curWord] & mask) {
        io_buffer.iv_DataStr[w] = '1';
      }
      else {
        io_buffer.iv_DataStr[w] = '0';
      }
  ...
  io_buffer.iv_XstateEnabled = true;
  return rc;
}
#endif /* REMOVE_SIM */
...
bool   ecmdDataBuffer::hasXstate(uint32_t i_start, uint32_t i_length) const {
    return hasXstate(*this, i_start, i_length);
}

bool   ecmdDataBuffer::hasXstate(const ecmdDataBufferBase & i_bufferIn, uint32_t i_start, uint32_t i_length) {
  if (!i_bufferIn.iv_XstateEnabled) {
    ETRAC("Xstate operation called on buffer without xstate's enabled");
    SET_ERROR_DATA(i_bufferIn.iv_RealData, ECMD_DBUF_XSTATE_NOT_ENABLED);
    return false;
  ...
  uint32_t minStop = i_bufferIn.iv_NumBits < stopBit ? i_bufferIn.iv_NumBits : stopBit; /* pick the smallest */

  for (uint32_t i = i_start; i < minStop; i++) {
    if (i_bufferIn.iv_DataStr[i] != '0' && i_bufferIn.iv_DataStr[i] != '1')
      return true;
  }
  return false;
}
...
uint32_t ecmdDataBuffer::setXstate(uint32_t i_bit, char i_value) {
  ...
  if (i_value == '0') rc = clearBit(i_bit);
    ...
    ETRAC("unrecognized Xstate character: %c", i_value);
    RETURN_ERROR(ECMD_DBUF_XSTATE_ERROR);
  }
...
uint32_t ecmdDataBuffer::setXstate(uint32_t i_bit, char i_value, uint32_t i_length) {
  ...
  if (!iv_XstateEnabled) {
    ETRAC("Xstate operation called on buffer without xstates enabled");
    RETURN_ERROR(ECMD_DBUF_XSTATE_NOT_ENABLED);
  }

  if (i_bit + i_length > iv_NumBits) {
    ETRAC("bit %d + len %d > NumBits (%d)", i_bit, i_length, iv_NumBits);
    RETURN_ERROR(ECMD_DBUF_BUFFER_OVERFLOW);
  }
...
uint32_t ecmdDataBuffer::setXstate(uint32_t i_bitOffset, const char* i_datastr) {
  ...
  if (i_bitOffset+len > iv_NumBits) {
    ETRAC("i_bitOffset %d + len %d > NumBits (%d)", i_bitOffset, len, iv_NumBits);
    RETURN_ERROR(ECMD_DBUF_BUFFER_OVERFLOW);
  }
  ...
  for (uint32_t i = 0; i < len; i++) {
    if (i_datastr[i] == '0') rc = clearBit(i_bitOffset+i);
      ...
      ETRAC("unrecognized Xstate character: %c", i_datastr[i]);
      RETURN_ERROR(ECMD_DBUF_XSTATE_ERROR);
    }
  }
...
uint32_t ecmdDataBuffer::memCopyInXstate(const char * i_buf, uint32_t i_bits) { /* Does a memcpy from supplied buffer into ecmdDataBuffer */
  ...
  if (buf_len < i_bits) {
    ETRAC("supplied buffer(%x) shorter than i_bits(%x)", buf_len, i_bits);
    RETURN_ERROR(ECMD_DBUF_INVALID_DATA_FORMAT);
  }
...
uint32_t ecmdDataBuffer::writeFileMultiple(const char * i_filename, ecmdFormatType_t i_format, ecmdWriteMode_t i_mode, uint32_t & o_dataNumber, const char * i_facName) {
 ...
 //Check if format asked for is the same as what was used b4
  ...
  if (iv_XstateEnabled) {
    if((hasXstate()) && (i_format != ECMD_SAVE_FORMAT_XSTATE)) {
      ETRAC("Buffer has Xstate data but non-xstate save mode requested");
      return(ECMD_DBUF_XSTATE_ERROR);
    }
  }
  #else
  if (i_format == ECMD_SAVE_FORMAT_XSTATE) {
    ETRAC("FORMAT_XSTATE not supported in this configuration");
    return(ECMD_DBUF_XSTATE_ERROR);
  }    
  #endif
 ...
 if (i_format == ECMD_SAVE_FORMAT_BINARY_DATA ) {
   if (i_mode != ECMD_WRITE_MODE) {
    ETRAC("File Format ECMD_SAVE_FORMAT_BINARY_DATA only supported in ECMD_WRITE_MODE");
    RETURN_ERROR(ECMD_DBUF_INVALID_ARGS);
   } else if (i_facName != NULL) {
    ETRAC("File Format ECMD_SAVE_FORMAT_BINARY_DATA does not support storing of string-property");
    RETURN_ERROR(ECMD_DBUF_INVALID_ARGS);
   }
 }

  if ((i_facName != NULL) && (strlen(i_facName) > 200)) {
    ETRAC("Input FacName string of length greater than 200 bytes not supported");
    RETURN_ERROR(ECMD_DBUF_INVALID_ARGS);
  } 
  ...
  if ((!ins.fail()) && (i_mode != ECMD_WRITE_MODE) && (i_format != ECMD_SAVE_FORMAT_BINARY_DATA)) {
    ...
    if (totalFileSz == 0) {
      ...
      if (existingFmt != i_format) {
        ETRAC("Format requested does not match up with the file Format.");
    RETURN_ERROR(ECMD_DBUF_INVALID_ARGS);
      }
      ins.seekg(begOffset);
      ...
      if (ins.fail()) {
       ETRAC("Read of the offset table failed on file : %s",i_filename);
  ...
  if (ops.fail()) {
    ETRAC("Unable to open file : %s for write",i_filename);
    if (offsetTableData != NULL) delete[] offsetTableData;
  ...
  if (i_format != ECMD_SAVE_FORMAT_BINARY_DATA) {
    ...
    if (ops.fail()) {
     ETRAC("Write of the header failed on file : %s",i_filename);
    if (offsetTableData != NULL) delete[] offsetTableData;
     RETURN_ERROR(ECMD_DBUF_FILE_OPERATION_FAIL); 
    }
  }
  ...
  if ( i_format == ECMD_SAVE_FORMAT_BINARY) {
    ...
    ops.write("END",3);//Key
    if (ops.fail()) {
     ETRAC("Write operation in format ECMD_SAVE_FORMAT_BINARY failed on file : %s",i_filename);
     if (offsetTableData != NULL) delete[] offsetTableData;
     RETURN_ERROR(ECMD_DBUF_FILE_OPERATION_FAIL); 
    }
  }  
   ...
   ops << "\nEND\n";
   if (ops.fail()) {
     ETRAC("Write operation in format ECMD_SAVE_FORMAT_ASCII failed on file : %s",i_filename);
     if (offsetTableData != NULL) delete[] offsetTableData;
     RETURN_ERROR(ECMD_DBUF_FILE_OPERATION_FAIL); 
   }
  } 
  ...
  if (!iv_XstateEnabled) {
   ...
   ops << "\nEND\n";
   if (ops.fail()) {
     ETRAC("Write operation in format ECMD_SAVE_FORMAT_XSTATE failed on file : %s",i_filename);
     if (offsetTableData != NULL) delete[] offsetTableData;
     RETURN_ERROR(ECMD_DBUF_FILE_OPERATION_FAIL); 
   }
  }
    ...
    delete[] buffer; buffer = NULL;
    if (ops.fail()) {
     ETRAC("Write operation in format ECMD_SAVE_FORMAT_BINARY_DATA failed on file : %s",i_filename);
     if (offsetTableData != NULL) delete[] offsetTableData;
     RETURN_ERROR(ECMD_DBUF_FILE_OPERATION_FAIL); 
    }
    ops.close();
...
uint32_t ecmdDataBuffer::queryNumOfBuffers(const char * i_filename, ecmdFormatType_t i_format, uint32_t &o_num) {
  ...
  if (ins.fail()) {
    ETRAC("Unable to open file : %s for reading",i_filename);
    RETURN_ERROR(ECMD_DBUF_FOPEN_FAIL);  
  }
  ...
  totalFileSz = ins.tellg();
  if (totalFileSz == 0) {
    ETRAC("File : %s is empty",i_filename);
    RETURN_ERROR(ECMD_DBUF_INVALID_ARGS);
  } else {
  ...
  if (!endFound) {
    ETRAC("END keyword not found. Invalid File Format.");
...
uint32_t ecmdDataBuffer::readFileMultiple(const char * i_filename, ecmdFormatType_t i_format, const char * i_prpty, uint32_t &o_dataNumber) {
  ...
  if (i_format == ECMD_SAVE_FORMAT_BINARY_DATA) {
    ETRAC("File Format ECMD_SAVE_FORMAT_BINARY_DATA not supported when property value is used as input.");
    RETURN_ERROR(ECMD_DBUF_INVALID_ARGS);
  }
  ...
  //Read the DataBuffer offset table-Seek to the correct DataBuffer Hdr
    ...
    delete [] dataOffsets;
    if (!propertyMatch) {
       ETRAC("Match for property: %s not found",i_property.c_str());
       RETURN_ERROR(ECMD_DBUF_INVALID_ARGS);
    } else {
      std::string ofac;
...
uint32_t ecmdDataBuffer::readFileMultiple(const char * i_filename, ecmdFormatType_t i_format, uint32_t i_dataNumber, std::string *o_facName) {
  ...
  if (i_dataNumber != 0) {
    ...
    if (i_format == ECMD_SAVE_FORMAT_BINARY_DATA) {
      ETRAC("File Format ECMD_SAVE_FORMAT_BINARY_DATA not supported when file contains multiple DataBuffers.");
      RETURN_ERROR(ECMD_DBUF_INVALID_ARGS);
    }
    ...
    if ((begOffset+8+(4*i_dataNumber)) >= endOffset) {
      ETRAC("Data Number requested exceeds the maximum Data Number in the File.");
    ...
    if (ins.fail()) {
     ETRAC("Read of the dataOffset failed on file : %s",i_filename);
     RETURN_ERROR(ECMD_DBUF_FILE_OPERATION_FAIL); 
    }
    ins.seekg(dataOffset);
    if (ins.eof()) {
     ETRAC("Data Offset is greater than the file size.");
     RETURN_ERROR(ECMD_DBUF_FILE_OPERATION_FAIL); 
    }
  }
  ...
  if ( i_format == ECMD_SAVE_FORMAT_BINARY) {
    ...
    if (i_format != ECMD_SAVE_FORMAT_BINARY ) {
      ETRAC("Format mismatch. Expected ECMD_SAVE_FORMAT_BINARY.");
      RETURN_ERROR(ECMD_DBUF_FILE_FORMAT_MISMATCH);  
    }
    ins.read((char *)&property,4);    property = htonl(property);
    if (property == 0x80000000) {
    ...
    if (ins.fail()) {
      ETRAC(" Read operation in format ECMD_SAVE_FORMAT_BINARY failed on file : %s",i_filename);
      delete[] buffer;
      RETURN_ERROR(ECMD_DBUF_FILE_OPERATION_FAIL); 
    }
    for(uint32_t i=0; i< getWordLength(); i++) {
    ...
    ins.read((char *)buffer,numBytes);
    if (ins.fail()) {
      ETRAC("Read operation in format ECMD_SAVE_FORMAT_BINARY_DATA failed on file : %s",i_filename);
      delete[] buffer;
      RETURN_ERROR(ECMD_DBUF_FILE_OPERATION_FAIL); 
    }
    for(uint32_t i=0; i< getWordLength(); i++) {
    ...
    if (i_format != ECMD_SAVE_FORMAT_ASCII ) {
      ETRAC("Format mismatch. Expected ECMD_SAVE_FORMAT_ASCII.");
      RETURN_ERROR(ECMD_DBUF_FILE_FORMAT_MISMATCH);  
    }
    ins.getline(hexstr, 9); 
    if (strncmp(hexstr, "80000000", 8) == 0) {
    ...
    i_format = (ecmdFormatType_t) strtoul(hexstr, NULL, 16);
    if (i_format != ECMD_SAVE_FORMAT_XSTATE ) {
      ETRAC("Format mismatch. Expected ECMD_SAVE_FORMAT_XSTATE.");
      RETURN_ERROR(ECMD_DBUF_FILE_FORMAT_MISMATCH);
    }
    ins.getline(hexstr, 9);
